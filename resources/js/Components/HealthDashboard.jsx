...
export default function HealthDashboard() {
    ...
    const fetchAlerts = async () => {
        try {
            ...
            if (res.status === 200) {
                const MedicalAlerts = res.data.alerts.filter(
                    (alert) => alert.alertType.toLowerCase() === "medical"
                );
                setAlerts(MedicalAlerts);
                setUsers(res.data.user);
                prepareChartData(MedicalAlerts);
                prepareAffectedAreas(MedicalAlerts); // Prepare data for the map
    ...
    // Filter Medical cases for the previous day, week, and month for comparison
        ...
        if (term.trim()) {
            ...
            try {
                ...
                if (response.data && response.data.status === 200 && Array.isArray(response.data.alerts)) {
                    const MedicalAlerts = response.data.alerts.filter(
                        (alert) => alert.alertType.toLowerCase() === "medical"
                    );

                    setSearchResults(MedicalAlerts);
                } else {
                            ...
                            <CardContent>
                                ...
                                {loading ? (<Loader />
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <p className="text-gray-600 dark:text-gray-300">No active Medical alerts.</p>
                                    ) : (
                                    <>
                                        <div className="space-y-3">
