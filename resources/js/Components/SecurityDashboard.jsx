...
export default function SecurityDashboard() {
    ...
    const fetchAlerts = async () => {
        try {
            ...
            if (res.status === 200) {
                const SecurityAlerts = res.data.alerts.filter(
                    (alert) => alert.alertType.toLowerCase() === "security"
                );
                setAlerts(SecurityAlerts);
                setUsers(res.data.user);
                prepareChartData(SecurityAlerts);
                prepareAffectedAreas(SecurityAlerts); // Prepare data for the map
    ...
    // Filter Security cases for the previous day, week, and month for comparison
        ...
        if (term.trim()) {
            ...
            try {
                ...
                if (response.data && response.data.status === 200 && Array.isArray(response.data.alerts)) {
                    const SecurityAlerts = response.data.alerts.filter(
                        (alert) => alert.alertType.toLowerCase() === "security"
                    );

                    setSearchResults(SecurityAlerts);
                } else {
    ...
    const prepareChartDataForApex = (data) => {
        ...
        return {
            ...
            options: {
                chart: {
                    ...
                    toolbar: { // Add a toolbar for interactions (optional)
                        ...
                        export: {
                            ...
                            svg: {
                                filename: 'Security_alerts_chart',
                            },
                            png: {
                                filename: 'Security_alerts_chart',
                            }
                ...
                title: {
                    text: 'Security Alerts Over Time',
                    align: 'left',
                    style: {
                        fontSize: '18px',
                        fontWeight: 'bold',
            ...
            <div className={`flex-1 p-6 space-y-8 dark:bg-gray-900 dark:text-gray-200 transition-colors duration-300`}>
                <div className="flex justify-between items-center">
                    <h1 className="text-3xl font-extrabold text-blue-900 dark:text-gray-100">
                        Emergency Response Security Dashboard 🔐
                    </h1>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <Card>
                        <CardContent>
                            <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-gray-100">Security Alerts Over Time</h2>
                            {chartData.length > 0 ? (
                                <ReactApexChart
